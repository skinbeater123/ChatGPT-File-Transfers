...
SOURCE="SOURCE_BIGANT_A01_LOCKED";WORK="DTC_SPRINT_A_WORK";BASES=("DTC_SPRINT_A_BASELINE_V01","DTC_SPRINT_A_BASELINE_V02","DTC_SPRINT_A_BASELINE_V03","DTC_SPRINT_A_BASELINE_V04");BASIS="DTC_X_FORWARD_Y_LEFT_Z_UP_METRES_NORMALIZED";VERSION="DTC_SPRINT_A_v05_BEULAH_SHELL";NATIVE_CHANGED=3838;NATIVE_MAX=.0214083495500494;NATIVE_MEAN=.01018749668792615
...
def main():
 ...
 f=one(w,'Frame_LOD0');f0=old(b4,f.name);sh=[(b-a).length for a,b in zip(pts(f0),pts(f))];active=[x for x in sh if x>1e-6];changed=len(active);mx=max(active);mean=sum(active)/changed;assert changed==NATIVE_CHANGED,changed;assert abs(mx-NATIVE_MAX)<5e-6,mx;assert abs(mean-NATIVE_MEAN)<5e-6,mean
 pe={}
 for o in meshes(w):
  if o is f:continue
 ...
 r={'schema':'dtc_sprint_a_v05_fresh_reload_validation_v1','status':'V05_FRESH_RELOAD_PASS','asset_version':VERSION,'authoring_basis':BASIS,'work_meshes':16,'v04_rollback_meshes':16,'changed_meshes':flags,'native_changed_vertices':changed,'native_max_vertex_shift_m':mx,'native_mean_changed_vertex_shift_m':mean,'representation_note':'Native Blender source has merged seam vertices; GLB export has additional split entries and is checked separately against the accepted preview.','max_protected_geometry_error_m':max(pe.values()),'wheel_pivot_errors_m':piv,'rollback_representation':b4.get('dtc.rollback_representation')}
