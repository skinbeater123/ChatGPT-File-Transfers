...
def main():
    ...
    g=bpy.context.object; g.name='REVIEW_GROUND'; mat=bpy.data.materials.new('Ground'); mat.diffuse_color=(0.12,0.12,0.12,1); g.data.materials.append(mat)
    ...
    sc.render.engine='BLENDER_EEVEE'; sc.render.resolution_x=1000; sc.render.resolution_y=700; sc.render.resolution_percentage=100
    sc.render.image_settings.file_format='PNG'; sc.render.film_transparent=False
    r=max(span.x,span.y,span.z)*1.65
    views={'front_3q':(ctr.x+r,ctr.y-r*.75,ctr.z+r*.52),'left_side':(ctr.x,ctr.y+r*1.45,ctr.z+r*.33),'right_side':(ctr.x,ctr.y-r*1.45,ctr.z+r*.33),'rear_3q':(ctr.x-r,ctr.y+r*.75,ctr.z+r*.52),'top_3q':(ctr.x+r*.4,ctr.y-r*.3,ctr.z+r*1.65)}
    for name,loc in views.items(): cam.location=loc; look_at(cam,ctr); sc.render.filepath=str(ns.out/f'{name}.png'); bpy.ops.render.render(write_still=True)
    info={'asset_version':sc.get('dtc.asset_version'),'bounds_min':list(lo),'bounds_max':list(hi),'objects':[{'name':o.name,'loc':list(o.matrix_world.translation)} for o in objs]}
    (ns.out/'review_info.json').write_text(json.dumps(info,indent=2)); print('DTC_V09_REVIEW_RENDER_PASS')
